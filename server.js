...
function verificarSesion(req, res, next) {
  if (!req.session.usuario) {
    return res
      .status(401)
      .json({ mensaje: "Debes iniciar sesión para continuar." });
  }
  next();
}
...
app.put("/api/usuario/:email", (req, res) => {
  const email = req.params.email;
  const { nombreCompleto, nombreUsuario, provincia, fechaNacimiento } =
    req.body;
...
app.post("/api/cambiar-contrasena", (req, res) => {
  ...
  fs.readFile(usuariosPath, "utf8", (err, data) => {
    if (err) {
      ...
      return res
        .status(500)
        .json({ mensaje: "Error interno al leer usuarios" });
    }
    ...
    try {
      ...
      return res
        .status(500)
        .json({ mensaje: "Error en el formato del archivo de usuarios" });
    }

    const index = usuarios.findIndex(
      (u) => u.usuario === correo && u.rol === rol
    );
    ...
    fs.writeFile(usuariosPath, JSON.stringify(usuarios, null, 2), (err) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({ mensaje: "No se pudo guardar la nueva contraseña" });
      }
...
app.post("/api/registro", (req, res) => {
  ...
  fs.readFile(usuariosPath, "utf-8", (err, data) => {
    if (err) {
      ...
      return res
        .status(500)
        .json({ mensaje: "Error al acceder a la base de usuarios" });
    }
    ...
    fs.writeFile(usuariosPath, JSON.stringify(usuarios, null, 2), (err) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({ mensaje: "No se pudo guardar el usuario" });
      }
      ...
      res
        .status(201)
        .json({ mensaje: "Registro exitoso", usuario: req.session.usuario });
    });
  });
});
...
app.post("/api/enviar-correo-recuperacion", (req, res) => {
  ...
  fs.readFile(usuariosPath, "utf8", (err, data) => {
    ...
    <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
      ...
      <a href="http://localhost:3000/html/establecerNuevaContra.html?email=${encodeURIComponent(
        email
      )}"
...
app.post("/carrito/agregar", (req, res) => {
  ...
  if (!user) {
    return res
      .status(401)
      .json({ mensaje: "Debes iniciar sesión para agregar al carrito." });
  }

  if (user.rol !== "comprador") {
    return res
      .status(403)
      .json({ mensaje: "Solo compradores pueden usar el carrito." });
  }
...
app.delete("/carrito/eliminar/:id", (req, res) => {
  ...
  if (user.rol !== "comprador") {
    return res
      .status(403)
      .json({ mensaje: "Solo compradores pueden modificar el carrito." });
  }
...
app.get("/carrito", (req, res) => {
  ...
  if (user.rol !== "comprador") {
    return res.status(403).json({
      mensaje: "Solo los usuarios compradores pueden ver el carrito.",
    });
  }
...
app.get("/api/juegos", async (req, res) => {
  ...
  try {
    ...
    const juegosConPrecio = response.data.results.map((juego) => ({
      ...juego,
